...
class RoutingTable(object):
	...
	def __msgDataHandling(self, index):
		retList = []
		retList.append(self.msgValidDataList[index][column_index_from_string('C') - 2])
		retList.append(str(len(self.msgDesChList[self.msgValidDataList.index(self.msgValidDataList[index])]) - 1) + 'u')
		retList.append("255u")
		if self.msgValidDataList[index][column_index_from_string('M') - 3] == '0':
			...
			for i in range(5 - len(self.msgDesChList[index]) + 1):
				...
				retList.append('0u')
		retList.append('0u')
		retList.append('0x0000u')
		retList.append(self.msgValidDataList[index][column_index_from_string('M') - 3])
		retList.append(self.msgValidDataList[index][column_index_from_string('N') - 3])
		retList.append(self.msgValidDataList[index][column_index_from_string('K') - 3])
	...
	def __sigDataHandling(self, index):
		retList = []
		retList.append(self.signalValidDataList[index][0])
		retList.append("0u")
		retList.append("255u")
		for i in range(5):
			retList.append('0u')
		...
		retList.append('0x0000u')
		retList.append('0')
		retList.append(self.signalValidDataList[index][2])
		retList.append(self.signalValidDataList[index][1])
	...
	def data_handle(self):
		...
		for subSigList in self.signalValidDataList:
			for subRouterList in self.routerTableList:
				if subSigList[0] == subRouterList[0]:
					subRouterList[-5] = str(SrcSig_Num) + 'u'
					subRouterList[-4] = "0x" + hex(int("0xF000", 16) + int(subRouterList[-4][:-1], 16) + 8*SrcSig_Num)[2:].upper() + 'u'
					SrcSig_Num += 1
		...
		for i in range(len(self.routerTableISRList)):
			for j in range(len(self.msgDesChListISR[i][1:])):
				self.routerTableISRList[i][3 + j*2] = str(MO_ISR_num) + 'u'
				MO_ISR_num += 1
		# print(self.routerTableISRList)
...
class PbDirectRoutingTable(RoutingTable, HexBase):
	...
	def build_table(self):
		"""创建数组表"""
		self.PB_DirectRoutingTable.append("const uint8 PB_Table1_Len __at(0x00c08001) = " + str(len(self.routerTableISRList)) + "u;\n")
...
class PbMsgRoutingTable(RoutingTable, HexBase):
	...
	def build_table(self):
		"""创建数组表"""
		self.PB_MsgRoutingTable.append("const uint8 PB_Table0_Len __at(0x00c0a46d) = " + str(len(self.routerTableFIFOList)) + "u;\n")
...
class PbMsgRecvTable(HexBase):
	...
	def build_table(self):
		"""创建数组表"""
		self.PB_Msg_Recv_Table.append("const uint8 u8PostBuildSingalTableLength __at(0x00c0a51f) = " + str(len(self.PBMsgRecvTableList)) + "u;\n")
...
class PbMsgSendTable(HexBase):
	...
	def __init__(self):
		...
		self.lenType = "uint8"
		self.structType = ["uint32", "uint16", "uint8", "uint8", "uint8", "uint8", "uint16", "uint16"] #最后一个为uint8，字节补全处理为uint16
		self.structLen = 14
		self.tableLenAddr = ""
		self.lenHexDataList = []
	...
	def build_table(self):
		"""创建数组表"""
		self.PB_Msg_Send_Table.append("const PB_TAB_TX_TYP PB_Msg_Send_Table[TABLE_45_SIZE] __at(0x00c08002) =\n")
